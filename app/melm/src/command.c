...
static void melm_cmd_vel_stop(struct context* ctx)
{
...
static int melm_command_cmd_handler(const struct shell* sh,
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_melm_command, melm_command_cmd_handler,
    (start, &g_ctx, "start"),
    (stop, &g_ctx, "stop"),
    (status, &g_ctx, "status"));
